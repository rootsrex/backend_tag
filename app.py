...
def consultar_placa(placa):
    try:
        ...
        if respuesta.status_code != 200:
            return jsonify({"status": "error", "mensaje": "Error de servicio"}), respuesta.status_code

        datos = respuesta.json()
        propietario = datos.get("nombre") or datos.get("propietario") or datos.get("titular") or datos.get("nombres") or "DATOS EN CONSTRUCCIÓN"
